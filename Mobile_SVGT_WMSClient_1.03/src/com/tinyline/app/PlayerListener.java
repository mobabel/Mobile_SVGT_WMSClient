 ...
 *          <p>
 */
public class PlayerListener implements org.w3c.dom.events.EventListener,
		AnimationCallback {
	 ...
	 * 
	 * @param seconds
	 *            The new current time in seconds relative to the start time for
	 ...
	 * causing the animation clock corresponding to this document fragment to
	...
	/**
	 * <b>SMIL:</b> Posts the SMIL event from an animation element to an
	 * interested party.
	 * 
	 * The SMIL event type can be one of
	 * <ul>
	 * <li> SVGAnimationElem.BEGIN_EVENT </li>
	 * <li> SVGAnimationElem.END_EVENT </li>
	 * <li> SVGAnimationElem.REPEAT_EVENT </li>
	 * </ul>
	 * 
	 * @param eventType
	 *            The SMIL event type
	...
	/**
	 * <b>uDOM:</b> Invoked whenever an event occurs of the type for which the
	 * <tt> EventListener</tt> interface was registered.
	 * 
	 * @param evt
	 *            The <tt>Event</tt> contains contextual information about the
	 *            event.
	 */
	public void handleEvent(Event evt) {
		...
		case SVGEvent.EVENT_UPDATE:
			...
			// System.out.println("UPDATE" + "[ " + data.xmin +"," + data.ymin
			// +","+ data.xmax+"," + data.ymax +"]");
		...
		case SVGEvent.EVENT_ANIMATE:
			...
			// System.out.println("document.nActiveAnimations: " +
			// document.nActiveAnimations );
		...
		case SVGEvent.EVENT_LOAD:
			...
			event = new SVGEvent(SVGEvent.EVENT_UPDATE, canvas.raster
					.getDevClip());
			canvas.postEvent(event);
			break;
		...
		case SVGEvent.EVENT_SCROLL:
			...
			// just quit
			if (!document.isZoomAndPanAnable())
				return;
		...
		case SVGEvent.EVENT_FOCUSHIDE:
			...
			if (document.linkTargets.count == 0)
				return;
		...
		case SVGEvent.EVENT_FOCUSIN:
			...
			if (node != null) {
				...
				event = new SVGEvent(SVGEvent.EVENT_UPDATE, node
						.getDevBounds(canvas.raster));
				canvas.postEvent(event);
			}
			break;
		...
		case SVGEvent.EVENT_FOCUSNEXT:
			...
			if (document.linkTargets.count == 0)
				return;
			if ((document.linkIndex + 1) == document.linkTargets.count)
				return;
			// hide the old link
